...
menu_data=[{'label':'Fixtures'},{'label':'Table'},{'label':'Update Fixture'},{'label':'Payment'}]
...
"""
    ...
    player_query = """SELECT name,payment_status FROM players;"""
    df = pd.read_sql(player_query, conn)

    st.subheader('Payment updates')
    option = st.radio('View',['Check Payment Status','Update Payment'],horizontal=True)
    if option == 'Update Payment':
        ...
        for i in payment:
            if i == 'Paid':
                ...
                y.append(0)
        df['y'] = y
        st.bar_chart(data=df,x='name', y='y',x_label='Status',horizontal=True)
else:
    conn = connection.connection()
    query = """
